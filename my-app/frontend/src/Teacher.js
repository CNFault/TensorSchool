	...
	render() {
		return (
			<div className='teacher' onClick={() => this.handlerClick()}>
				...
				<span title="ВУЗ Курс">{this.state.position}, {this.state.age} лет</span>
			</div>)
	}

	handlerClick() {
		super.handlerClick(<span className="popup_info_position">Стаж работы - {this.state.expirience} лет</span>) // наш опциональный контент
	}
}
